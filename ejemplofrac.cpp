...
class Semana_Laboral{
 	public:
 		Semana_Laboral(char A[20],double);
 		~Semana_Laboral();
 		void sethoraingreso(char A[9]);
 		void sethorasalida(char B[9]);
 		void getlaburo();
 	private:
 		char N[20];
 		double dni;
 	...
 	Semana_Laboral::Semana_Laboral(char A[20],double b){ 		
 		strcpy (N,A);
 		dni=b;
	 }
	...
	void Semana_Laboral::sethoraingreso(char A[9]){
		Tiempo t;
		int h,m,s;
		cin>>h>>m>>s;
	...
	void Semana_Laboral::sethorasalida(char B[9]){
		Tiempo t;
		int h,m,s;
...
void Semana_Laboral::getlaburo(){
		...
		Tm=(S[0][1].obtieneMinuto()-S[0][0].obtieneMinuto())+(S[1][1].obtieneMinuto()-S[1][0].obtieneMinuto())+(S[2][1].obtieneMinuto()-S[2][0].obtieneMinuto())+(S[3][1].obtieneMinuto()-S[3][0].obtieneMinuto())+(S[4][1].obtieneMinuto()-S[4][0].obtieneMinuto());
 		cout<<Th<<" : "<<Tm<<endl;
 		cout<<S[0][1].obtieneHora1()<<S[0][0].obtieneHora1()<<endl;
 };
...
{  
	{
	Semana_Laboral a("juan",41642397);
		a.sethoraingreso("Lunes");
	//	a.sethoraingreso("Martes");
		a.sethorasalida("Lunes");
		//a.sethorasalida("Martes");
		a.getlaburo();
	}
